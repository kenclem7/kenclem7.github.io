...
| Open-Meteo forecast API, `models=gfs_hrrr` | HRRR short-range overlay, first ~2 days (orange dashes, toggleable; the toggle label carries a title tooltip explaining HRRR - on phones, where hover does not exist, the word HRRR in the label is a blue link opening the same text as a centered card, #hrrrhelp, with "10-day model" reworded to "daily model" since phones show one day at a time) | same |
