...
3. Share links: `?lat&lon&name&admin1&cc&elev` params override the saved city without touching it. The **share** button (a 34px square icon button right of the recents pull-down since 2026-08-18, both pages; flashes ✓ when the link is copied; the locmeta row keeps only "refresh") copies such a link for the current view.
...
Recents are a pull-down under the search row, in a flex row (#recrow) with the square share button at its right end: #recwrap (the pull-down + its menu) takes the remaining width, so together they span the search column. The button shows the active city with a caret, the menu lists up to 8 recents (active row highlighted, per-row × remove), shared across all three pages via `bw_recents`. Outside click or Escape closes it.
