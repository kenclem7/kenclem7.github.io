...
Recents are a pull-down under the search row, in a flex row (#recrow) with the square share button at its right end: #recwrap (the pull-down + its menu) takes the remaining width, so together they span the search column. The button shows the active city with an 18px caret (was 12px, too small per Ken 2026-08-18), the menu lists up to 8 recents (active row highlighted, per-row × remove), shared across all three pages via `bw_recents`. Outside click or Escape closes it.
