...
**Daily mode (phones, under 600px, added 2026-08-17):** the forecast pages stop scrolling sideways and show **one day at a time**. The chart fits the screen width (300px floor instead of 1080); `VIEW0`/`VIEWH` define the visible hour window and `x()`/`xb()` map hours into it; series paths draw the window plus one hour of overhang each side and are clipped to the plot by `clipPath#pclip`; loops (arrows, night shading, day strip, boundary lines, now line) are window-guarded. A **sticky `◀ date ▶` day pager** (#daypager, `position: sticky`) sits above the chart in place of the sticky date header; `viewDay` is in-memory only (not persisted), starts at today, resets on city change but not on the 30-minute auto-refresh, and clamps to the fetched days (10 on weather10, 3 on weather3). Y-axis scales stay computed over the whole fetched period, so paging days never rescales an axis (and the wind axis still tops at the period's max gust). Long legends stack one item per line as a block hugging the right edge of the plot: the color swatches share one aligned column and the text runs ragged on the right, never right-justified per line (per Ken 2026-08-18). The block is drawn left-aligned inside a `g.lgstack`, then shifted right by measured text width after render (character estimates are too coarse to land the edge). The tide legend shortens the station to its first comma-part on phones. Reorder/resize handles work unchanged. The h1 reads "City Daily Weather Forecast" in daily mode (browser tab title unchanged).
