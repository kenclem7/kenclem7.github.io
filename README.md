# kenclements.com

Personal site. Full details in [DESIGN.md](DESIGN.md).

- `/` redirects to [Ken's LinkedIn profile](https://www.linkedin.com/in/kenclements/)
- `/weather10/` is a personal 10-day forecast page (Wunderground-style charts, NOAA NBM + HRRR data via [Open-Meteo](https://open-meteo.com/), non-commercial use with attribution); `/weather/` and `/weather1/` redirect there
- `/weather3/` is the same forecast page limited to today, tomorrow, and the day after
- `/weather365/` shows a city's average year (10-year daily temperature and precipitation averages, ERA5 via Open-Meteo); `/weather2/` redirects there

Hosted on GitHub Pages.
